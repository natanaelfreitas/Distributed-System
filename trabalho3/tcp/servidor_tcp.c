...
int main(void){
	...
	char *mensagem = "O servidor está bem e você?\0";   
 ...
 //   char filename[1024] = {0}; 
    ...
    /*    if (recv(conexao, filename, 1024, 0) == -1) 
        {
            perror("Can't receive filename");
            exit(1);
        }*/
        FILE *fp = fopen("/run/media/natanael/Data/trabalho3/testetcp.avi", "wb");
        if (fp == NULL) 
        {
            perror("Can't open file");
            exit(1);
        }
        writeFile(conexao,fp);
